    ...
    public void live() throws InterruptedException {
        Thread thread = new Thread() {
            ...
            private void someActions() throws InterruptedException {
                eat();
                play();
                // должен быть вызван метод sleep класса Solution.
//                new Solution(name, food, sound).sleep(1000); // работает
                Solution.this.sleep(1000);
                // или надо через рефлексию!?
//                Thread.currentThread().getClass().getConstructors(name, food, sound);
//                this(name, food, sound).sleep(1000); // this most bee first ioni constructor
//                sleep(1000);
            }
        };
        thread.start();
